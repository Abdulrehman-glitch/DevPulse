...
`scripts/test-authenticode-verification.ps1` runs only inside a disposable GitHub-hosted Windows runner. It verifies the runner's trusted Microsoft-signed PowerShell executable as an actual `Valid` fixture, then creates a one-day, self-signed, non-exportable **TEST** key in the runner's CurrentUser `My` store to verify an actual unsigned file, an intact untrusted signed file, and a hash-mismatched/tampered file. It removes the key in `finally` and discards its temporary directory. A deterministic classifier self-test also covers Windows' expiry, unsupported-format, and unknown-error mappings without changing a protected trust store. Hosted-runner attempts to anchor a self-signed certificate in CurrentUser `Root` proved interactive or unbounded and are deliberately excluded. The released installer remains unsigned; an actual `Valid` result for DevPulse itself is a production-signing activation gate once an approved publicly trusted identity exists. No certificate or key is uploaded. The result validates machinery only and is never production code-signing assurance.
